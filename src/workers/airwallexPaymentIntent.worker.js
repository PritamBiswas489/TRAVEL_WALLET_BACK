...
worker.on("completed", (job) => {
  console.log(`✅ ${job.name} completed for intent ${job.data.intentId} (job ${job.id})`);
});
...
worker.on("failed", async (job, err) => {
  ...
  if (job.name === JOB_NAMES.FUND_SPLIT) {
    ...
    try {
      ...
      process.env.SENTRY_ENABLED === "true" && Sentry.captureException(enqueueErr);
    }
  }
...
console.log(`👷 Airwallex PaymentIntent worker started (queue: ${AIRWALLEX_QUEUE_NAME})`);
